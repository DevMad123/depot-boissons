...
@section('admin-content')
    <div class="page-header">
        <div class="page-title">
            <h4>Liste des Emballages</h4>
            <h6>Gérez vos Emballages</h6>
        </div>
        <div class="page-btn">
            <a href="{{ route('admin.emballages.create') }}" class="btn btn-added"><img src="{{ asset('backend/assets/img/icons/plus.svg') }}"
                    alt="img" class="me-1">Nouveau Emballage</a>
        </div>
    </div>
                ...
                <table class="table  datanew table-striped" id="example">
                    <thead>
                        <tr>
                            ...
                            <th>Libelle</th>
                            <th class="text-center">Qte par emballage</th>
                    ...
                    <tbody>
                        @foreach ($emballages as $emballages)
                            <tr>
                                ...
                                <td>{{ $emballages->matriemb }}</td>
                                <td>{{ $emballages->libelle }}</td>
                                <td class="text-center">{{ $emballages->qte_par_emballage }}</td>
                                <td class="text-center">{{ \Carbon\Carbon::parse($emballages->created_at)->format('d-m-Y à H:i') }}</td>
                                <td>
                                    @if (auth()->user()->can('admin.delete'))
                                        <a class="confirm-text" href="javascript:void(0);"
                                                onclick="event.preventDefault(); if(confirm('Êtes vous sûr de vouloir supprimer?')) { document.getElementById('delete-form-{{ $emballages->id }}').submit(); }">
                                            <img src="{{ asset('backend/assets/img/icons/delete.svg') }}" alt="img">
                                        </a>

                                        <form id="delete-form-{{ $emballages->id }}"
                                            action="{{ route('admin.emballages.destroy', $emballages->id) }}" method="POST"
                                            style="display: none;">
                                            @method('DELETE')
                                            @csrf
                                        </form>
