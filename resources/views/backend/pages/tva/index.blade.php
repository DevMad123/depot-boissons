...
@section('admin-content')
    <div class="page-header">
        ...
        <div class="page-btn">
            <a href="{{ route('admin.tvas.create') }}" class="btn btn-added"><img src="{{ asset('backend/assets/img/icons/plus.svg') }}"
                    alt="img" class="me-1">Nouvelle Tva</a>
        </div>
    </div>
        ...
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table class="table  datanew table-striped" id="example">
                    ...
                    <tbody>
                        @foreach ($tvas as $tvas)
                            <tr>
                                <td  class="text-center">{{ $loop->index+1 }}</td> 
                                    <td  class="text-center">{{ $tvas->taux.' '.$tvas->symbol }}</td>
                                    ...
                                    <td  class="text-center">{{ \Carbon\Carbon::parse($tvas->created_at)->format('d-m-Y à H:i') }}</td>
                                <td  class="text-center">
                                    @if ($tvas->status ==1)
                                        Activé
                                        @else
                                        ...
                                        <a class="btn btn-success" style="color: #ffff;"  href="javascript:void(0);"
                                                onclick="event.preventDefault(); if(confirm('Êtes vous sûr de vouloir activer ?')) { document.getElementById('delete-form-{{ $tvas->id }}').submit(); }">
                                                Activer
                                            </a>

                                        <form id="delete-form-{{ $tvas->id }}"
                                            action="{{ route('admin.tvas.destroy', $tvas->id) }}" method="POST"
                                            style="display: none;">
                                            @method('DELETE')
                                            @csrf
                                        </form>
