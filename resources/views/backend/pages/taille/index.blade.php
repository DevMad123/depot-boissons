...
@section('admin-content')
    <div class="page-header">
        <div class="page-title">
            <h4>Liste du Format</h4>
            <h6>Gérez vos  Format</h6>
        </div>
        <div class="page-btn">
            <a href="{{ route('admin.tailles.create') }}" class="btn btn-added"><img src="{{ asset('backend/assets/img/icons/plus.svg') }}"
                    alt="img" class="me-1">Nouvelle  Format</a>
        </div>
    </div>
                    ...
                    <tbody>
                        @foreach ($formats as $format)
                            <tr>
                                ...
                                <td class="text-center">{{ $format->format }}</td>
                                <td class="text-center">{{ \Carbon\Carbon::parse($format->created_at)->format('d-m-Y à H:i') }}</td>
                                <td>
                                    @if (auth()->user()->can('admin.delete'))
                                        <a class="confirm-text" href="javascript:void(0);"
                                                onclick="event.preventDefault(); if(confirm('Êtes vous sûr de vouloir supprimer?')) { document.getElementById('delete-form-{{ $format->id }}').submit(); }">
                                            <img src="{{ asset('backend/assets/img/icons/delete.svg') }}" alt="img">
                                        </a>

                                        <form id="delete-form-{{ $format->id }}"
                                            action="{{ route('admin.tailles.destroy', $format->id) }}" method="POST"
                                            style="display: none;">
                                            @method('DELETE')
                                            @csrf
                                        </form>
