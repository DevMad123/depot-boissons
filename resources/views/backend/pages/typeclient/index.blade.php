...
@section('admin-content')
    <div class="page-header">
        <div class="page-title">
            <h4>Liste des Types Client</h4>
            <h6>Gérez vos  Types Client</h6>
        </div>
        <div class="page-btn">
            <a href="{{ route('admin.typeclients.create') }}" class="btn btn-added"><img src="{{ asset('backend/assets/img/icons/plus.svg') }}"
                    alt="img" class="me-1">Nouveau  Type Client</a>
        </div>
    </div>
                    ...
                    <tbody>
                        @foreach ($typeclients as $typeclients)
                            <tr>
                                ...
                                <td >{{ $typeclients->type }}</td>
                                <td >{{ $typeclients->observation }}</td>
                                <td class="text-center">{{ \Carbon\Carbon::parse($typeclients->created_at)->format('d-m-Y à H:i')  }}</td>
                                <td>
                                    <a class="me-3" href="product-details.html">
                                        <img src="{{ asset('backend/assets/img/icons/eye.svg') }}" alt="img">
                                    </a>
                                     @if (auth()->user()->can('admin.edit'))
                                        <a class="me-3" href="{{ route('admin.admins.edit', $typeclients->id) }}"><img
                                                src="{{ asset('backend/assets/img/icons/edit.svg') }}" alt="img"></a>
                                    @endif
                                    @if (auth()->user()->can('admin.delete'))
                                        <a class="confirm-text" href="javascript:void(0);"
                                                onclick="event.preventDefault(); if(confirm('Êtes vous sûr de vouloir supprimer?')) { document.getElementById('delete-form-{{ $typeclients->id }}').submit(); }">
                                            <img src="{{ asset('backend/assets/img/icons/delete.svg') }}" alt="img">
                                        </a>

                                        <form id="delete-form-{{ $typeclients->id }}"
                                            action="{{ route('admin.typeclients.destroy', $typeclients->id) }}" method="POST"
                                            style="display: none;">
                                            @method('DELETE')
                                            @csrf
                                        </form>
